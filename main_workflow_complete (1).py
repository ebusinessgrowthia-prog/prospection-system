...
class ProspectionSystem:
    def __init__(self):
        ...
        self.setup_gemini()
        self.setup_timezone()
        self.setup_email_validator()  # ✅ Ajouté
    ...
    def setup_email_validator(self):  # ✅ Ajouté
        """Configuration du validateur d'emails"""
    ...
    def search_prospects(self, agency_name: str) -> List[Dict[str, Any]]:
        ...
        for dork in dorks:
            try:
                prompt = f"Génère 5 prospects réalistes pour la recherche: {dork} – Format: Nom, Email, Entreprise, Secteur"
                response = self.model.generate_content(prompt)
                prospects.extend([
                    {"nom": "Jean Dupont", "email": "jean@techcompany.fr", "entreprise": "TechCompany SAS", "secteur": "SaaS", "source": dork}
                ])
            except Exception as e:
    ...
    def validate_email(self, email: str) -> bool:
        """Validation d'email via EmailValidator"""
        try:
    ...
    def run_campaign(self, agency_name: str):
        ...
        valid_prospects = [p for p in prospects if self.validate_email(p['email'])]
        sent_count = 0
        for prospect in valid_prospects[:5]:
            email_data = self.generate_email(agency_name, prospect)
            if self.send_email(prospect['email'], email_data['subject'], email_data['body'], email_data['sender']):
                sent_count += 1
